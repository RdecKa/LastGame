...
public class GameClient extends Thread
{
	...
	public GameClient() throws Exception {
		...
		try {
			...
			Message con = new Message(nickname, "__server__", null, null);
	...
	private void sendMessage(Message message, ObjectOutputStream out) {
		try {
			out.writeObject(message); // send the message to the chat server
...
class GameClientMessageReceiver extends Thread {
	...
	public void run() {
		try {
			Message message;
			while ((message = (Message)this.in.readObject()) != null) { // read new message
